...
@interface TD_FMDatabase ()

- (TD_FMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;

@end

@implementation TD_FMDatabase
...
static int FMDBDatabaseBusyHandler(void *f, int count) {
    TD_FMDatabase *self = (__bridge TD_FMDatabase*)f;
...
- (void)closeOpenResultSets {
    ...
    for (NSValue *rsInWrappedInATastyValueMeal in openSetCopy) {
        TD_FMResultSet *rs = (TD_FMResultSet *)[rsInWrappedInATastyValueMeal pointerValue];
...
- (void)resultSetDidClose:(TD_FMResultSet *)resultSet {
...
- (TD_FMStatement*)cachedStatementForQuery:(NSString*)query {
    
    NSMutableSet* statements = [_cachedStatements objectForKey:query];
    
    return [[statements objectsPassingTest:^BOOL(TD_FMStatement* statement, BOOL *stop) {
...
- (void)setCachedStatement:(TD_FMStatement*)statement forQuery:(NSString*)query {
...
- (BOOL)goodConnection {
    ...
    TD_FMResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (TD_FMResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments {
...
- (TD_FMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    TD_FMStatement *statement  = 0x00;
    TD_FMResultSet *rs         = 0x00;
    ...
    if (!statement) {
        statement = [[TD_FMStatement alloc] init];
    ...
    rs = [TD_FMResultSet resultSetWithStatement:statement usingParentDatabase:self];
    [rs setQuery:sql];
...
- (TD_FMResultSet *)executeQuery:(NSString*)sql, ... {
    va_list args;
    va_start(args, sql);
...
- (TD_FMResultSet *)executeQueryWithFormat:(NSString*)format, ... {
    va_list args;
...
- (TD_FMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments {
    return [self executeQuery:sql withArgumentsInArray:arguments orDictionary:nil orVAList:nil];
}

- (TD_FMResultSet *)executeQuery:(NSString*)sql withVAList:(va_list)args {
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    int rc                   = 0x00;
    sqlite3_stmt *pStmt      = 0x00;
    TD_FMStatement *cachedStmt  = 0x00;
    ...
    if (_shouldCacheStatements && !cachedStmt) {
        cachedStmt = [[TD_FMStatement alloc] init];
