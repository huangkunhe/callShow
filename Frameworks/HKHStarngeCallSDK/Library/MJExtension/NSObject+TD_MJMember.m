...
@implementation NSObject (TD_MJMember)
...
- (void)enumerateIvarsWithBlock:(MJIvarsBlock)block
{
    [self enumerateClassesWithBlock:^(__unsafe_unretained Class c, BOOL *stop) {
        ...
        for (int i = 0; i<outCount; i++) {
            TD_MJIvar *ivar = [[TD_MJIvar alloc] initWithIvar:ivars[i] srcObject:self];
            ivar.srcClass = c;
            block(ivar, stop);
        }
...
- (void)enumerateMethodsWithBlock:(MJMethodsBlock)block
{
    [self enumerateClassesWithBlock:^(__unsafe_unretained Class c, BOOL *stop) {
        ...
        // 2.遍历每一个成员变量
        for (int i = 0; i<outCount; i++) {
            TD_MJMethod *method = [[TD_MJMethod alloc] initWithMethod:methods[i] srcObject:self];
            method.srcClass = c;
            block(method, stop);
        }
