...
#define kWaterAnimateDuration   6.0
...
@property (weak, nonatomic) UIButton *descriptionLbl;
...
//        NSLog(@"%f",frame.size.width);
        ...
        UIButton *descriptionLbl=[[UIButton alloc]init];
...
//        UILabel *descriptionLbl = [[UILabel alloc]init];
        ...
        descriptionLbl.titleLabel.font =[UIFont systemFontOfSize:13*W/250];
        [descriptionLbl setTitle:@"总评分" forState:UIControlStateNormal];
        descriptionLbl.titleLabel.textAlignment=NSTextAlignmentCenter;
        [descriptionLbl addTarget:self action:@selector(chickdescriptionTxt) forControlEvents:UIControlEventTouchUpInside];
...
-(void)chickdescriptionTxt
{
    if ([self.delegate respondsToSelector:@selector(chickdescriptionTxtDelegate:)]) {
        [self.delegate chickdescriptionTxtDelegate:self.descriptionLbl.titleLabel.text];
    }
}
...
- (void)setTextColor:(UIColor *)textColor{
    ...
    if (textColor) {
        ...
        self.descriptionLbl.tintColor = _textColor;
    }
}
...
- (void)setDescriptionTxt:(NSString *)descriptionTxt{
    ...
    if(descriptionTxt){
        [self.descriptionLbl setTitle:_descriptionTxt forState:UIControlStateNormal] ;
    }
}
...
- (void)addAnimateWithType:(int)type
{
    ...
    void(^acallBack)(CGFloat start) = ^(CGFloat start) {
        ...
        moveAction.removedOnCompletion = NO;
